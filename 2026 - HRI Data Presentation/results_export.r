...
df_report <- df_benthic_fma_sites %>%
    left_join(df_benthic_lcc_sites, by = c("Year", "Site")) %>%
    left_join(df_fish_biomass_H_site, by = c("Year", "Site")) %>%
    left_join(df_fish_biomass_C_site, by = c("Year", "Site")) %>%
    ungroup() %>%
    select(Year, Site,
...
write.csv(df_report, "2026 - HRI Data Presentation/outputs/report_2026.csv")
