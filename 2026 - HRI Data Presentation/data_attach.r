...
if (dir.exists("2026 - HRI Data Presentation/data_deposit") && length(list.files("2026 - HRI Data Presentation/data_deposit")) > 0) {
    message("Data files already present in 2026 - HRI Data Presentation/data_deposit folder")
} else {
    message("Downloading data from Dryad...")
    downloaded_files <- dryad_download(dois = "10.5061/dryad.c866t1gcn")
    file.copy(unlist(downloaded_files), "2026 - HRI Data Presentation/data_deposit", overwrite = TRUE)
    message("Files copied to 2026 - HRI Data Presentation/data_deposit/")
}
...
ref_biomass <- read.csv("2026 - HRI Data Presentation/data_provided/fish_biomass_specification.csv") %>%
    ...
    na.omit()
ref_fish_species <- read.csv("2026 - HRI Data Presentation/data_deposit/Ref_Fish_Species.csv", na.strings = na_strings)
ref_organisms_benthic <- read.csv("2026 - HRI Data Presentation/data_deposit/Ref_Organisms_Benthic.csv", na.strings = na_strings) %>%
